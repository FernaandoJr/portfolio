...
import {
	...
	HoverCardContent,
	HoverCardTrigger,
} from "@/components/ui/new-hover-card"
import {
	Timeline,
	TimelineConnector,
	TimelineContent,
...
function ExpandableText({ text }: { text: string }) {
	...
	return (
		<div className="mt-3">
			<p ref={ref} className={`text-sm text-muted-foreground leading-relaxed ${expanded ? "" : "line-clamp-2"}`}>
				{text}
			</p>
			{(clamped || expanded) && (
				<Tooltip>
										...
										<div className="flex items-center gap-2">
											<div className="size-7 shrink-0 flex items-center justify-center rounded-md border border-[color:var(--hc-border)] p-0.5">
												{exp.companyInfo.logo ? (
													<Image src={exp.companyInfo.logo} alt={exp.companyInfo.name} width={28} height={28} className="size-7 object-contain" />
												) : (
							...
							<Tooltip key={tag}>
								<TooltipTrigger>
									<Image
										...
										height={18}
										draggable={false}
										className={"size-[18px] cursor-pointer " + (skill.darkInvert ? "dark:invert" : "")}
									/>
								</TooltipTrigger>
