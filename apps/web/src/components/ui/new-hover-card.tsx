...
const HoverCard = ({
	...
	const containsInteractiveNode = React.useCallback((node: Node | null) => {
		return Boolean(
			node && (triggerRef.current?.contains(node) || contentRef.current?.contains(node))
		);
	}, []);
...
const HoverCardTrigger = React.forwardRef<HTMLButtonElement, HoverCardTriggerProps>(
	(
		{ asChild, className, onBlur, onFocus, onPointerEnter, onPointerLeave, type, ...props },
		ref
	) => {
		const {
			open,
			...
			<HoverCardPrimitive.Trigger asChild>
				<Comp
					...
					onBlur={(event) => {
						...
						handleFocusEnd(event as React.FocusEvent<HTMLElement>);
					}}
					onFocus={(event) => {
					...
					onPointerEnter={(event) => {
						...
						if (event.defaultPrevented) return;
						handleHoverStart(event as React.PointerEvent<HTMLElement>);
					}}
					onPointerLeave={(event) => {
						onPointerLeave?.(event as React.PointerEvent<HTMLButtonElement>);
						if (event.defaultPrevented) return;
						handleHoverEnd(event as React.PointerEvent<HTMLElement>);
					}}
			...
			>
				<motion.div
					...
					onBlur={(event) => {
						onBlur?.(event);
						if (event.defaultPrevented) return;
						handleFocusEnd(event as unknown as React.FocusEvent<HTMLElement>);
					}}
					onFocus={(event) => {
						onFocus?.(event);
						if (event.defaultPrevented) return;
					...
					onPointerEnter={(event) => {
						onPointerEnter?.(event);
						if (event.defaultPrevented) return;
						handleHoverStart(event as unknown as React.PointerEvent<HTMLElement>);
					}}
					onPointerLeave={(event) => {
						onPointerLeave?.(event);
						if (event.defaultPrevented) return;
						handleHoverEnd(event as unknown as React.PointerEvent<HTMLElement>);
					}}
