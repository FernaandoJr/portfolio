...
const fillHoles = (activities: Activity[]): Activity[] => {
	...
	return eachDayOfInterval({
		...
		return calendar.has(date)
			? (calendar.get(date) as Activity)
...
const getMonthLabels = (
	...
	return weeks
		...
		.filter(({ weekIndex }, index, labels) => {
			const min = 3
			if (index === 0)
				return labels[1] && labels[1].weekIndex - weekIndex >= min
			if (index === labels.length - 1)
				return weeks.slice(weekIndex).length >= min
			return true
		})
}
...
export const ContributionGraphBlock = ({
	...
	if (activity.level < 0 || activity.level > maxLevel) {
		throw new RangeError(
			`Level ${activity.level} out of range [0, ${maxLevel}].`
		)
	}
	return (
		<rect
...
export const ContributionGraphLegend = ({
	...
	return (
		<div
			className={cn("ml-auto flex items-center gap-0.5 select-none", className)}
			{...props}>
