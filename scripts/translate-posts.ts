...
import { fileNameFor, serializeVariant } from "../src/lib/blog/parse";
import type { Frontmatter, Locale } from "../src/lib/blog/types";
import { validateTranslation } from "../src/lib/blog/validate";
import { CONTENT_DIR, hashOf, planJobs, type Job } from "./posts-io";
...
const SYSTEM_INSTRUCTION = `You are a professional technical translator for a software engineer's personal blog.

Translate the given blog post faithfully, preserving the author's voice: direct, opinionated, conversational but precise. Do not soften strong claims, do not add hedging, do not add or remove content.
...
async function translate(ai: GoogleGenAI, job: Job): Promise<TranslationResult> {
	...
	const prompt = [
		`Translate this blog post from ${LANGUAGE_NAME[job.source.locale]} to ${LANGUAGE_NAME[job.target]}.`,
		"",
...
async function run() {
	...
	const jobs = await planJobs(force, only);

	if (jobs.length === 0) {
		log("✓ every post is translated and up to date");
		return;
	}
	...
	log(`translating ${jobs.length} file(s) with ${MODEL}\n`);
	...
	for (const job of jobs) {
		...
		try {
			...
			await writeFile(
				...
				serializeVariant(frontmatter, result.body),
				"utf8"
			);

			log(`  ✓ ${label}\n    wrote content/blog/${file}`);
		} catch (error) {
			failed += 1;
			...
			if (/not[_ ]found|not supported|no longer available|invalid.*model/i.test(message)) {
				log(`\n    "${MODEL}" was rejected. Run: pnpm blog:translate --models`);
				break;
			}
		}
	}
